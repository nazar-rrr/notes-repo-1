...
const removingTasks = (event) => {
    if (event.target.matches('.vectors-container__item--delete')) {
        ...
        const tasksEditButtonItem = document.querySelector('.tasks__item--edit');
        const itemVectorsContainer = document.querySelector('.tasks__item--vectors-container');

        if (itemVectorsContainer.classLisst.contains('tasks__item--vectors-container-appear')) {
